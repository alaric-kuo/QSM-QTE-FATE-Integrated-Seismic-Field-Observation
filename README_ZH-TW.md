...
本儲存庫將量子結構力學（QSM）、量子拓樸快遞方法（QTE），以及碎形生機拓樸演化論（FATE）的第一個營運層，應用於 NEES-2011-1076 三層鋼構架實驗的四組既有紀錄。
...
| 階段 | 算符與資訊狀態 | 目前工程角色 |
|---|---|---|
| **QSM** | 零對角哈密頓量： $H_{\mathrm{QSM}}=-W$ 。固定關係通道；僅邊界輸入；不進行持續樓層狀態同化 | 建立輸入驅動的關係型能勢狀態基線 |
| **QTE** | 拉普拉斯哈密頓量： $H_{\mathrm{QTE}}=L(W)$ 。動態路徑；僅邊界輸入；無持續反應回饋 | 恢復空間拓樸，觀測路徑權重與邊流演化 |
...
|---|---|---|
| **RPG** | 提供 $a\cdot v$ 作為直接可計算的單位質量功率表達，以及質量邊界語言 | 由實測運動形成帶正負號、經質量正規化且與作功相容的能勢輸入 |
...
|---|---|---|---|
| **RPG** | 單位質量功率與質量邊界語言 | 使用加速度與速度形成 $a\cdot v$ | 未取得實體樓層質量，因此不宣稱絕對瓦特 |
...
$$
H_{\mathrm{field}} = \kappa \cdot L_{\mathrm{geo}}(W) + \alpha_v \cdot \mathrm{diag}\left(V_{\mathrm{bg}}\right)
$$
...
$$
\frac{\mathrm{d}W}{m} \approx a\cdot\mathrm{d}u = a\cdot v\cdot\mathrm{d}t
$$
...
| $\hat{H}$ | 結構通道法則的哈密頓描述 |
| $\hat{H}_{p}$ | 哈密頓能勢轉換算符 |
| $\lvert\Psi(t)\rangle$ | 持續演化的結構能勢狀態 |
| $e^{\hat{H}_{p}t}$ | 狀態演化算子 |
...
$$
P(t) = (a\cdot v)\cdot\hat{H}_{p}\lvert\Psi(t)\rangle
$$

若寫成空間狀態：

$$
P(x,t) = (a\cdot v)\cdot\hat{H}_{p}\lvert\Psi(x,t)\rangle
$$
...
| QSM 物件 | 角色 |
|---|---|
| $a\cdot v$ | 直接動態能勢輸入，也是單位質量功率尺度 |
| $\hat{H}_{p}$ | 能勢通過結構通道時的轉換 |
| $\lvert\Psi(t)\rangle$ | 持續演化的結構能勢狀態 |

實驗應用可寫成：

$$
P(t) \sim P_{\mathrm{input}}(t)\cdot\hat{H}_{p}\lvert\Psi(t)\rangle
$$
...
$$
P_{\mathrm{real}}^{\mathrm{target}}(t) \sim P_{\mathrm{input}}(t)\cdot\mathrm{Fid}^{\mathrm{target}}(t)
$$

累積目標命中等效作功為：

$$
W_{\mathrm{hit}}^{\mathrm{target}}(T) = \int_0^T P_{\mathrm{input}}(t)\cdot\mathrm{Fid}^{\mathrm{target}}(t)\cdot\mathrm{d}t
$$
...
|---|---|
| 理論上的哈密頓通道法則 | $\hat{H}$ |
| QSM 哈密頓能勢轉換算符 | $\hat{H}_{p}=-i(\hat{H}/\hbar)$ |
| 程式層級的等效關係矩陣 | $H=-W$ |
| 程式層級的單步演化算子 | $U(\Delta t)=e^{-iH\Delta t}$ |
...
| 零對角通道條件 | 已實作 | $H=-W$ |
| 哈密頓狀態演化 | 已數值實作 | $U=e^{-iH\Delta t}$ |
| 結構複數狀態 | 以量測編碼實作 | 由實測或衍生的 $u$ 、 $v$ 與實測 $a$ 編譯 |
| 目標態保真度 | 已實作 | 完整狀態重疊與節點模平方 |
| 帶正負號的目標命中顯化 | 以作功相容量重建 | 保真度加權的 $a\cdot v$ 顯化 |
...
$$
H = \kappa \cdot L_{\mathrm{geo}} + \alpha_v \cdot \mathrm{diag}\left(V_{\mathrm{bg}}\right)
$$
...
$$
J_{ij}(t) = 2\cdot\mathrm{Im}\left(\psi_i^*(t)\cdot H_{ij}\cdot\psi_j(t)\right)
$$

局部速度場可寫成：

$$
v_i(t) = \frac{\sum_j J_{ij}(t)\cdot u_{ij}}{\rho_i(t)+\epsilon}
$$
...
$$
m_i(t) = m_i(0)\cdot\exp\left(\int_0^t \Gamma_i(\tau)\cdot\mathrm{d}\tau\right)
$$

完整顯化分數可寫成：

$$
M_i(t) = \lambda_1\cdot\left\lvert\Delta V_{\mathrm{resp},i}(t)\right\rvert + \lambda_2\cdot\max\left(\Gamma_i(t),0\right) + \lambda_3\cdot\max\left(\Delta V_{\mathrm{bg},i},0\right) + \lambda_4\cdot\log\left(m_i(t)\right)
$$
...
$$
J_{12}(t) = 2\cdot\mathrm{Im}\left(\psi_1^*(t)\cdot H_{12}\cdot\psi_2(t)\right)
$$

$$
J_{23}(t) = 2\cdot\mathrm{Im}\left(\psi_2^*(t)\cdot H_{23}\cdot\psi_3(t)\right)
$$
...
| $D_p>0$ | `1F–2F` 路徑顯影較強 |
| $D_p<0$ | `2F–3F` 路徑顯影較強 |
| $\lvert D_p\rvert\le 0.02$ | 路徑狀態接近均分 |
...
| 邊流 $J_{12}$ 、 $J_{23}$ | 已實作 |
| 路徑權重與主導指標 | 已實作 |
| 實體背景場 $V_{\mathrm{bg}}$ | 尚未實作 |
| 完整竣工場驅動哈密頓量 | 尚未實作 |
| 典範 $\Delta V_{\mathrm{resp}}$ 直接場輸出 | 尚未實作 |
| 顯化速率 $\Gamma$ | 尚未實作 |
| 累積顯化量 $m$ | 尚未實作 |
| 治理分數 $M$ | 尚未實作 |
| BIM／IFC 構件層級拓樸 | 尚未實作 |
...
| **竣工／試運轉** | 經確認的幾何、斷面、質量、剛度、阻尼、接頭、裝置、測試與邊界 | 由實際建築建立 $V_{\mathrm{bg}}$ 與場驅動哈密頓量 |
| **營運** | 感測器、裝置狀態、應變、層間位移、位移、速度、加速度、巡檢、維護與損傷 | 持續更新 $\Psi(t)$ 、 $W(t)$ 、 $V_{\mathrm{bg}}(t)$ 與 $H(t)$ ，將感知連接至介入與再演化 |
...
| 案例 | FATE 最終 $D_p$ | FATE 平均 $D_p$ | FATE 邊流比 $J_{12}/J_{23}$ | 平均顯化作功比例 | 最大反應樓層 |
|---|---:|---:|---:|---:|---|
...
| **QTE** | 拉普拉斯樓層拓樸、動態路徑、路徑主導與邊流 | 竣工 $V_{\mathrm{bg}}$ 、完整場驅動哈密頓量、典範顯化場與經驗證的構件定位 |
